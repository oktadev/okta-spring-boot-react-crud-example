...
. Add the necessary Spring Security dependencies to do OIDC authentication. [`spring-security-auth0`]
+
[source,xml]
----
<dependency>
...
. Modify `app/src/Home.js` to call `/api/user` to see if the user is logged in. If they're not, show a `Login` button. [`sbr-home-auth`]
+
====
[source,jsx]
----
